...
Инструмент для автоматической миграции проектов Bevy Engine между версиями 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18 и 0.19.

## 📋 Описание

Этот инструмент предназначен для автоматизации процесса миграции проектов, использующих игровой движок Bevy, между различными версиями. Инструмент поддерживает миграцию от версии 0.12 до 0.19 с использованием модульной архитектуры и AST-анализа кода.

### Основные возможности

- 🔄 **Автоматическая миграция** между версиями Bevy 0.12 → 0.13 → 0.14 → 0.15 → 0.16 → 0.17 → 0.18 → 0.19
...
- 🧠 **Поддержка сложной логики** через Python callback-и и inline YAML правила
...
- **0.18 → 0.19 (Часть 1)**: Messages API, переименования наблюдателей, реорганизация reflect и основные переименования (Только ручной выбор цели)
...
# Установите пакет в редактируемом режиме (рекомендуется для разработки)
pip install -e .
...
Это установит пакет `bevymigrate` и команду CLI `bevymigrate`.

## 🚀 Как запустить

### Базовое использование

После установки вы можете запускать инструмент напрямую с помощью команды `bevymigrate`:

```bash
# Запуск миграции вашего проекта (0.18 — цель по умолчанию)
bevymigrate /путь/к/проекту/bevy

# Явный выбор миграции до Bevy 0.19 (Часть 1)
bevymigrate /путь/к/проекту/bevy --target-version 0.19-part1

# Запуск в режиме предварительного просмотра (dry-run)
bevymigrate /путь/к/проекту/bevy --dry-run
```

```bash
# Миграция проекта до последней поддерживаемой версии (0.18)
bevymigrate /путь/к/вашему/bevy/проекту

# Миграция до конкретной версии
bevymigrate /путь/к/вашему/bevy/проекту --target-version 0.17

# Предварительный просмотр изменений (без модификации файлов)
bevymigrate /путь/к/вашему/bevy/проекту --dry-run

# Подробный вывод с отладочной информацией
bevymigrate /путь/к/вашему/bevy/проекту --verbose
```
...
bevymigrate /путь/к/проекту --backup-dir ./мои_резервные_копии

# Принудительная миграция (если версия не определяется автоматически)
bevymigrate /путь/к/проекту --force

# Исключение определенных файлов или директорий
bevymigrate /путь/к/проекту --exclude "target/**" "*.bak" "custom_dir/**"
...
└── bevymigrate/
    ...
    │   ├── v0_14_to_0_15_part1.py  # Миграция 0.14 → 0.15 Часть 1
    │   ├── v0_14_to_0_15_part2.py  # Миграция 0.14 → 0.15 Часть 2
    │   ├── v0_15_to_0_16.py        # Миграция 0.15 → 0.16
    │   ├── v0_16_to_0_17_part1.py  # Миграция 0.16 → 0.17 Часть 1
    │   ├── v0_16_to_0_17_part2.py  # Миграция 0.16 → 0.17 Часть 2
    │   ├── v0_16_to_0_17_part3.py  # Миграция 0.16 → 0.17 Часть 3
    │   ├── v0_17_to_0_18.py        # Миграция 0.17 → 0.18
    │   └── v0_18_to_0_19_part1.py  # Миграция 0.18 → 0.19 Часть 1
    └── utils/                      # Утилиты
...
Для трансформаций, требующих большего, чем простая замена по паттерну, инструмент предоставляет два мощных механизма: Python callback-и и inline YAML правила. Они применяются напрямую без создания временных файлов на диске.

### 1. Python Callback-и
Используются для условных замен, динамической манипуляции строками или трансформаций, зависящих от контекста.

```python
def input_callback(vars, file_path):
    # vars содержит захваченные переменные ast-grep (например, $T)
...
transformations.append(self.create_transformation(
    pattern="Res<Input<$T>>",
    replacement="", # Игнорируется при наличии callback
    description="Сложная замена через Python",
    callback=input_callback
))
```

### 2. Inline YAML правила
Позволяют использовать всю мощь конфигурации `ast-grep` напрямую.
...
transformations.append(self.create_transformation(
    pattern="", # Игнорируется при наличии rule_yaml
    replacement="",
    description="Продвинутое YAML правило",
    rule_yaml=yaml_rule
))
```
def input_callback(vars, file_path):
...
# В get_transformations:
transformations.append(self.create_transformation(
    pattern="Res<Input<$T>>",
    replacement="", # replacement игнорируется при наличии callback
