...
- 🧠 **Поддержка сложной логики** через Python callback-и для продвинутых трансформаций
...
## 🧠 Сложные миграции (Callback-и)

Для трансформаций, которые требуют больше, чем простая замена по паттерну, вы можете использовать Python callback-и. Это полезно для:
- Условных замен на основе захваченных переменных
- Динамических манипуляций со строками (например, изменение регистра)
- Трансформаций, зависящих от контекста

### Пример: Динамическая миграция Input
```python
